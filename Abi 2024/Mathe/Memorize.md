...
~~~
### Grenzwert an einer Stelle $x\to x_0$
~~~ad-important
...
- Die Tangente schneidetn den Graphen an weiteren Punkten.
...
```mermaid
graph LR
	__[menu]-->A[4: Analysis]-->a[1: Ableitung]
	__[menu]-->B[3: Algebra]-->b[1: Löse]
...
$f(x):= ...$
$f1(x):= ...$
$solve(f1(x)=(2-f(x))/(-x), x)\to2\sqrt{2}\quad-2\sqrt{2}$
...
```mermaid
graph LR
	__[menu]-->A[4: Analysis]-->a[A: Normalterm]
```
$normalline(f, x, x_0)$
~~~
